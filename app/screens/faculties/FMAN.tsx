  ...
  useEffect(() => {
    const fetchInstructors = async () => {
      try {
        ...
        // Filter for SBS faculty (which is the same as FMAN)
        const sbsFaculty = teachersList.filter(teacher => 
          teacher.faculty === "Sabancı Business School"
        );
        
        setInstructors(sbsFaculty);
        setInstructorCount(sbsFaculty.length);
        setLoading(false);
      } catch (error) {
